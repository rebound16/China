...
readdata <- function(file, pos, name){
  pos_n <- pos+1 # cnennames includes the column "provinces", therefore the variable of interest will be column 4
  ...
  jilin <- as.data.frame(merged[which(tolower(merged$cityname_e) == tolower(merged$provinces)),][4]) # Jilin is both province and city, therefore the row with the province (larger value or first value, if they are the same) needs to be removed if they both occur.
  merged_n <- if (nrow(jilin) == 2){
    if (all(jilin[1, ] == jilin[2, ])){
      merged[-as.numeric(rownames(jilin)[1]), ]
    } else {
      merged[-which(tolower(merged$cityname_e) == "jilin" & merged[, 4] == max(jilin)), ]
    }
  } else {
    merged
  }
